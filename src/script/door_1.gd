	...
	if Input.is_action_just_pressed("klik_kiri"):
		...
		if player_infront and $AnimationPlayer.is_playing() == false:
			...
			if door_state == true:
				...
				get_node("hinge/MeshInstance2/StaticBody/CollisionShape").disabled = true
			else:
				$AnimationPlayer.play_backwards("door1")
				get_node("hinge/MeshInstance2/StaticBody/CollisionShape").disabled = false
